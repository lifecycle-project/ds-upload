...
lc.reshape.core <- local(function(upload_to_opal = TRUE, data_version, input_format = 'CSV', input_path, output_path = getwd()) {
  ...
  lc.reshape.core.generate.trimester.repeated(lc_data, upload_to_opal, output_path, file_prefix, dict_kind, file_version, 'trimester_repeated_measures')
...
lc.reshape.core.generate.non.repeated <- local(function(lc_data, upload_to_opal, output_path, file_prefix, dict_kind, file_version, file_name) {
  ...
  non_repeated <- c(lc.variables.primary.keys(), lc.variables.core.non.repeated())
...
lc.reshape.core.generate.monthly.repeated <- local(function(lc_data, upload_to_opal, output_path, file_prefix, dict_kind, file_version, file_name) {
  ...
  long_1 <- monthly_repeated_measures %>% 
    gather(orig_var, height_, lc.variables.core.monthly.repeated(), na.rm=TRUE)
...
#' Generate the trimester repeated measures file and write it to your local workspace
#'
#' @param lc_data data frame with all the data based upon the CSV file
#' @param upload_to_opal do you want to upload to Opal (default = true)
#' @param output_path directory where the CSV files need to be stored
...
#' @importFrom tidyr gather spread
#' 
lc.reshape.core.generate.trimester.repeated <- local(function(lc_data, upload_to_opal, output_path, file_prefix, dict_kind, file_version, file_name) {
  # workaround to avoid glpobal variable warnings, check: https://stackoverflow.com/questions/9439256/how-can-i-handle-r-cmd-check-no-visible-binding-for-global-variable-notes-when
  orig_var <- smk_t <- age_trimesters <- NULL
  
  message('* Generating: trimester-repeated measures')
  
  # Select the non-repeated measures from the full data set
  trimester_repeated <- c(lc.variables.primary.keys(), lc.variables.core.trimester.repeated())
  trimester_repeated_measures <- lc_data[,trimester_repeated]
  
  if(nrow(lc.data.frame.remove.all.na.rows(trimester_repeated_measures)) <= 0) {
    message('* WARNING: No trimester-repeated measures found in this set')
    return()
  } 
  
  # First re-arrange the whole data set to long format, unspecific for variable
  long_1 <- trimester_repeated_measures %>% 
    gather(orig_var, smk_t, lc.variables.core.trimester.repeated(), na.rm=TRUE)
  ...
  # Arrange the variable names based on the original order
  long_trimester <- long_2[,c("row_id", "child_id", "age_years", "age_trimesters", unique(long_1$variable_trunc))]
  ...
  # Subset of data with age_months = 0
  zero_trimester <- long_trimester %>%
    filter(age_trimesters %in% 0)
  
  # Subset of data with age_months > 0
  long_trimester <- long_trimester %>%
    filter(age_trimesters > 0)
  
  write_csv(long_trimester, paste(output_path, '/', file_prefix, '_', dict_kind, '_', file_version, '_', file_name, '.csv', sep=""), na = "")
