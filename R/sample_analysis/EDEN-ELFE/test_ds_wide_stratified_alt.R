...
## Create a variable describing the gender:

ds.Boole.o(V1 = 'D$sex', V2 = '1' , Boolean.operator = '==', newobj = 'is_boy')
ds.cbind(c('is_boy', 'D'), newobj = 'D')

ds.Boole.o(V1 = 'D$sex', V2 = '1' , Boolean.operator = '==', newobj = 'is_girl')
ds.cbind(c('is_girl', 'D'), newobj = 'D')

## Create an overweight variable from the data and the cutoffs table: Boys
...
for(i in array){
  ...
  dest_var <- paste("overweight_boys_", i, sep ="")
  ...
  # Is the cuttof gender correct?
  
  ds.make.o(toAssign = paste(dest_var, "* D$is_boy", sep =""), dest_var)
...
## Create an overweight variable from the data and the cutoffs table: girls
...
for(i in array){
  ...
  dest_var <- paste("overweight_girls_", i, sep ="")
  
  cutoffs_var <- as.character(cutoffs[as.character(i), "girls"])
  
  # Logical test: is the individual above or below the cuttof for his age and gender?:
  
  ds.Boole.o(V1 = initial_var, V2 = cutoffs_var , Boolean.operator = '>=', newobj = dest_var)
  
  # Is the cuttof gender correct?
  
  ds.make.o(toAssign = paste(dest_var, "* D$is_girl", sep =""), dest_var)
...
# Get an unified per month overweight variable:

avoid <- vector()

for(i in array){
  
  # Define the gendered overweight variables
  
  initial_var_girls <- paste("overweight_girls_", i, sep ="")
  
  initial_var_boys <- paste("overweight_boys_", i, sep ="")
  
  dest_var <- paste("overweight_", i, sep ="") 
  
  if(ds.class(paste("D$", initial_var_girls, sep = "")) != "list" & 
     ds.class(paste("D$", initial_var_boys, sep = "")) != "list"){
    
    # And sum them:
    
    ds.make.o(toAssign = paste(initial_var_girls, "+", initial_var_boys, sep = ''), dest_var)
