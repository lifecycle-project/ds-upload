...
du.reshape <- local(function(upload_to_opal = TRUE, data_version, input_format, dict_version, 
    ...
    du.reshape.generate.non.repeated(lc_data, upload_to_opal, output_path, file_prefix, 
        dict_kind, data_version, "non_repeated_measures")
    du.reshape.generate.yearly.repeated(lc_data, upload_to_opal, output_path, file_prefix, 
        dict_kind, data_version, "yearly_repeated_measures")
    du.reshape.generate.monthly.repeated(lc_data, upload_to_opal, output_path, file_prefix, 
        dict_kind, data_version, "monthly_repeated_measures")
    
    if (dict_kind == "outcome") {
        du.reshape.generate.weekly.repeated(lc_data, upload_to_opal, output_path, file_prefix, 
            dict_kind, data_version, "weekly_repeated_measures")
    }
    
    if (dict_kind == "core" & dict_version != "1_0") {
        du.reshape.generate.trimesterly.repeated(lc_data, upload_to_opal, output_path, 
            file_prefix, dict_kind, data_version, "trimester_repeated_measures")
    }
...
du.reshape.generate.non.repeated <- local(function(lc_data, upload_to_opal, output_path, 
    ...
    non_repeated_measures <- data.frame(row_id = c(1:length(non_repeated_measures$child_id)), 
        non_repeated_measures)
    
    # Write as csv
    write_csv(non_repeated_measures, paste(output_path, "/", file_prefix, "_", dict_kind, 
        "_", file_version, "_", file_name, ".csv", sep = ""), na = "")
...
du.reshape.generate.yearly.repeated <- local(function(lc_data, upload_to_opal, output_path, 
    ...
    lc_variables_yearly_repeated_dict <- du.retrieve.dictionaries("yearly_rep", dict_kind)
    matched_columns <- du.match.columns(colnames(lc_data), lc_variables_yearly_repeated_dict$name)
    ...
    for (id in unique(yearly_repeated_measures$child_id)) {
        if (!(id %in% zero_year$child_id)) {
            zero_year %<>% summarise(child_id = id, age_years = 0) %>% bind_rows(zero_year, 
                .)
        }
    }
    ...
    write_csv(long_yearly, paste(output_path, "/", file_prefix, "_", dict_kind, "_", 
        file_version, "_", file_name, ".csv", sep = ""), na = "")
...
du.reshape.generate.monthly.repeated <- local(function(lc_data, upload_to_opal, output_path, 
    ...
    lc_variables_monthly_repeated_dict <- du.retrieve.dictionaries("monthly_rep", dict_kind)
    matched_columns <- du.match.columns(colnames(lc_data), lc_variables_monthly_repeated_dict$name)
    monthly_repeated_measures <- lc_data[, matched_columns]
    ...
    for (id in unique(monthly_repeated_measures$child_id)) {
        if (!(id %in% zero_monthly$child_id)) {
            zero_monthly %<>% summarise(child_id = id, age_months = 0) %>% bind_rows(zero_monthly, 
                .)
        }
    }
    ...
    write_csv(long_monthly, paste(output_path, "/", file_prefix, "_", dict_kind, "_", 
        file_version, "_", file_name, ".csv", sep = ""), na = "")
...
du.reshape.generate.weekly.repeated <- local(function(lc_data, upload_to_opal, output_path, 
    ...
    lc_variables_weekly_repeated_dict <- du.retrieve.dictionaries("weekly_rep", dict_kind)
    matched_columns <- du.match.columns(colnames(lc_data), lc_variables_weekly_repeated_dict$name)
    weekly_repeated_measures <- lc_data[, matched_columns]
    ...
    for (id in unique(weekly_repeated_measures$child_id)) {
        if (!(id %in% zero_weekly$child_id)) {
            zero_weekly %<>% summarise(child_id = id, age_weeks = 0) %>% bind_rows(zero_weekly, 
                .)
        }
    }
    ...
    write_csv(long_weekly, paste(output_path, "/", file_prefix, "_", dict_kind, "_", 
        file_version, "_", file_name, ".csv", sep = ""), na = "")
...
du.reshape.generate.trimesterly.repeated <- local(function(lc_data, upload_to_opal, output_path, 
    ...
    lc_variables_trimesterly_repeated_dict <- du.retrieve.dictionaries("trimester_rep", 
        dict_kind)
    matched_columns <- du.match.columns(colnames(lc_data), lc_variables_trimesterly_repeated_dict$name)
    trimesterly_repeated_measures <- lc_data[, matched_columns]
    ...
    for (id in unique(trimesterly_repeated_measures$child_id)) {
        if (!(id %in% one_trimesterly$child_id)) {
            one_trimesterly %<>% summarise(child_id = id, age_trimester = 1) %>% bind_rows(one_trimesterly, 
                .)
        }
    }
    ...
    write_csv(long_trimesterly, paste(output_path, "/", file_prefix, "_", dict_kind, 
        "_", file_version, "_", file_name, ".csv", sep = ""), na = "")
